...
declare global {
  namespace jest {
    interface Matchers<R> {
      readonly toMatchRightObject: (expected: unknown) => R;
    }
  }
}

const passMessage = <R>(received: Either<unknown, string | R>, expected: RegExp | Partial<R>) => () =>
...
const failMessage = <R>(received: Either<unknown, string | R>, expected: RegExp | Partial<R>) => () =>
 ...
 */
export const toMatchRightObject = <R>(received: Either<unknown, R>, expected: Partial<R>): jest.CustomMatcherResult => {
