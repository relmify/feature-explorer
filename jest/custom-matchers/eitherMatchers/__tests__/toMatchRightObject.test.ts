...
describe('.toMatchRightObject should pass', () => {
  ...
  test('if received is a Right and expected is a subset of the right object', () => {
    expect(right({ name: 'Right', motto: 'Right is the best' })).toMatchRightObject({ motto: 'Right is the best' });
  });
...
describe('.toMatchRighttObject should fail', () => {
...
describe('.not.toMatchRighttObject should fail', () => {
  test('if received is a Right that matches the expected value', () => {
    expect(() =>
      expect(right({ first: 'Albert', last: 'Einstein' })).not.toMatchRightObject({ last: 'Einstein' }),
    ).toThrowError();
  });
});
