...
export const containsMatch = (
  ...
  const matchesItem = (expected: string | RegExp) => (received: string): boolean => {
    return typeof expected === 'string' ? received.includes(expected) : new RegExp(expected).test(received);
  };
